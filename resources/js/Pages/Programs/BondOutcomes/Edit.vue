...
const form = useForm({
    ...
    baseline_year: props.outcome.baseline_year,
    target_year: props.outcome.target_year,
    source_document: props.outcome.source_document,
    responsible_entity: props.outcome.responsible_entity,
    presidential_priority_ids: props.outcome.presidential_priorities?.map(p => p.id) || [],
    sectoral_goal_ids: props.outcome.sectoral_goals?.map(g => g.id) || [],
    nlgas_pillar_ids: props.outcome.nlgas_pillars?.map(p => p.id) || [],
})

const submit = () => {
    form.put(route('programs.bond-outcomes.update', props.outcome.id))
}
</script>

<template>
    <BeLayout>
        <Head title="Edit Bond Outcome" />

        <div class="row">
            <div class="col-lg-10">
                <h5 class="mt-4">Edit Bond Outcome</h5>
                <hr />
                        ...
                        <form @submit.prevent="submit">
                            ...
                            <div class="d-flex gap-2 mt-4">
                                <button type="submit" class="btn btn-success" :disabled="form.processing">
                                    <i class="bi bi-check-circle"></i> Update Outcome
                                </button>
