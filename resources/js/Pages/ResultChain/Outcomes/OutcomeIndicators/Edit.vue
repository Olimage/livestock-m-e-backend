...
const form = useForm({
    ...
    impact_indicator_ids: [...props.selectedImpacts],
    sectoral_goal_ids: [...props.selectedGoals],
})
...
const filteredImpacts  = computed(() => { const q = impactSearch.value.toLowerCase(); return q ? props.impactIndicators.filter(i => i.code.toLowerCase().includes(q) || i.title.toLowerCase().includes(q)) : props.impactIndicators })
const filteredGoals    = computed(() => { const q = goalSearch.value.toLowerCase(); return q ? props.sectoralGoals.filter(i => i.code.toLowerCase().includes(q) || i.title.toLowerCase().includes(q)) : props.sectoralGoals })
...
const submit = () => form.put(route('result-chain.outcome-indicators.update', props.indicator.id))
</script>

<template>
    <BeLayout>
        <Head title="Edit Outcome Indicator" />
        <h5 class="mt-4">Edit Outcome Indicator</h5>
        <hr />
            ...
            <div class="col-lg-11">
                <div class="card shadow-sm">
                    <div class="card-header card-header-green text-white">
                        <h6 class="mb-0"><i class="bi bi-graph-up me-2"></i>Edit Outcome Indicator — <span class="opacity-75">{{ indicator.code }}</span></h6>
                    </div>
                    <div class="card-body">
                        <form @submit.prevent="submit">
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-4">
                                    <label class="form-label fw-semibold">Impact Indicators <span class="text-danger">*</span> <small class="text-muted fw-normal d-block">(at least 1)</small></label>
                                    <div class="input-group input-group-sm mb-2"><span class="input-group-text"><i class="bi bi-search"></i></span><input v-model="impactSearch" type="text" class="form-control" placeholder="Filter..." /></div>
                                    <div class="indicator-list border rounded p-2" :class="{ 'border-danger': form.errors.impact_indicator_ids }">
                                        ...
                                        <div v-for="ind in filteredImpacts" :key="ind.id" class="form-check py-1 px-3 indicator-item" :class="{ selected: form.impact_indicator_ids.includes(ind.id) }">
                                            <input class="form-check-input" type="checkbox" :id="`imp-${ind.id}`" :value="ind.id" v-model="form.impact_indicator_ids" />
                                            <label class="form-check-label w-100" :for="`imp-${ind.id}`">
                                                <span class="badge badge-imp me-2">{{ ind.code }}</span><span class="small">{{ ind.title }}</span>
                                            </label>
                                        </div>
                                    </div>
                                ...
                                <div class="col-md-3 mb-4">
                                    ...
                                    <div class="input-group input-group-sm mb-2"><span class="input-group-text"><i class="bi bi-search"></i></span><input v-model="supportingDeptSearch" type="text" class="form-control" placeholder="Filter..." /></div>
                                    <div class="indicator-list border rounded p-2">
                                        ...
                                        <div v-for="d in filteredSupportingDepts" :key="d.id" class="form-check py-1 px-3 indicator-item" :class="{ selected: form.supporting_department_ids.includes(d.id) }">
                                            <input class="form-check-input" type="checkbox" :id="`sd-${d.id}`" :value="d.id" v-model="form.supporting_department_ids" />
                                ...
                                <div class="col-md-3 mb-4">
                                    ...
                                    <div class="input-group input-group-sm mb-2"><span class="input-group-text"><i class="bi bi-search"></i></span><input v-model="goalSearch" type="text" class="form-control" placeholder="Filter..." /></div>
                                    <div class="indicator-list border rounded p-2">
                                        <div v-if="!filteredGoals.length" class="text-muted small p-2">No goals match.</div>
                                        <div v-for="goal in filteredGoals" :key="goal.id" class="form-check py-1 px-3 indicator-item" :class="{ selected: form.sectoral_goal_ids.includes(goal.id) }">
                                            <input class="form-check-input" type="checkbox" :id="`sg-${goal.id}`" :value="goal.id" v-model="form.sectoral_goal_ids" />
                                            <label class="form-check-label w-100" :for="`sg-${goal.id}`">
                                                <span class="badge badge-goal me-2">{{ goal.code }}</span><span class="small">{{ goal.title }}</span>
                                            </label>
                                        </div>
                                    </div>
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <button type="submit" class="btn btn-success" :disabled="form.processing">
                                    <i class="bi bi-save me-2"></i>Update Outcome Indicator
                                </button>
                            </div>
                        </form>
                    </div>
