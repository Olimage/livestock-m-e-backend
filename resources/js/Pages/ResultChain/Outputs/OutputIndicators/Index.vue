...
watch([search, perPage, sortBy, sortOrder], () => {
    ...
    searchTimeout.value = setTimeout(() => {
        router.get(route('result-chain.output-indicators.index'), {
...
const del = (id) => {
    if (confirm('Delete this output indicator?')) {
        router.delete(route('result-chain.output-indicators.destroy', id), { preserveScroll: true })
    }
}
...
const toggleSort = (col) => {
    sortBy.value === col
        ...
        : (sortBy.value = col, sortOrder.value = 'asc')
}
const getSortIcon = (col) => sortBy.value !== col ? 'bi bi-arrow-down-up' : (sortOrder.value === 'asc' ? 'bi bi-arrow-up' : 'bi bi-arrow-down')
</script>

<template>
    <BeLayout>
        <Head title="Output Indicators" />

        <h5 class="mt-4 fw-400">Output Indicators</h5>
        <hr />
        <p class="text-muted">Prefix: <span class="badge badge-opt">OPT</span> &nbsp; Total: <strong>{{ totalCount }}</strong></p>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="card stat-card mb-3">
                                <div class="card-header">Total Output Indicators</div>
                                <div class="card-body"><h3 class="text-white">{{ totalCount }}</h3></div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-2 ms-auto">
                            <Link :href="route('result-chain.output-indicators.create')" class="btn btn-success w-100">
                                <i class="bi bi-plus-circle me-1"></i> Add Output Indicator
                            </Link>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <tr v-if="!indicators?.data?.length">
                                <td colspan="6" class="text-center text-muted py-4">No output indicators found</td>
                            </tr>
                            <tr v-for="ind in indicators?.data" :key="ind.id">
                                <td><span class="badge badge-opt">{{ ind.code }}</span></td>
                                <td>{{ ind.title }}</td>
                                <td><small>{{ ind.department?.name ?? '—' }}</small></td>
                                <td><span class="badge bg-secondary">{{ ind.activities_count ?? 0 }}</span></td>
                                <td><span class="badge bg-secondary">{{ ind.outcome_indicators_count ?? 0 }}</span></td>
                                <td>
                                    <div class="btn-group">
                                        <Link :href="route('result-chain.output-indicators.edit', ind.id)" class="btn btn-sm btn-outline-primary">
                                            <i class="bi bi-pencil"></i>
                                        </Link>
                                        <button @click="del(ind.id)" class="btn btn-sm btn-outline-danger">
...
.badge-opt { background-color: rgb(11,109,23); color: #fff; }
