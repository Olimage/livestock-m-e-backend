...
watch([search, perPage, sortBy, sortOrder], () => {
    ...
    t.value = setTimeout(() => {
        router.get(route('result-chain.program-output-indicators.index'), {
...
const del = (id) => {
    if (confirm('Delete this Program Output Indicator?'))
        router.delete(route('result-chain.program-output-indicators.destroy', id), { preserveScroll: true })
}
...
<template>
    <BeLayout>
        <Head title="Program Output Indicators" />
        <h5 class="mt-4 fw-400">Program Output Indicators (PPOI)</h5>
        <hr />
        <p class="text-muted">Prefix: <span class="badge badge-ppoi">PPOI</span> &nbsp; Total: <strong>{{ totalCount }}</strong></p>

        <div class="row card mb-3">
            <div class="col-sm-12">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4"><div class="card stat-card mb-3"><div class="card-header">Total PPOI</div><div class="card-body"><h3 class="text-white">{{ totalCount }}</h3></div></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-5 mb-2">
                            <div class="input-group"><span class="input-group-text"><i class="bi bi-search"></i></span>
                            <input v-model="search" type="text" class="form-control" placeholder="Search..." /></div>
                        </div>
                        <div class="col-md-2 mb-2">
                            <select v-model="perPage" class="form-select"><option :value="10">10 / page</option><option :value="25">25 / page</option></select>
                        </div>
                        <div class="col-md-3 ms-auto mb-2">
                            <Link :href="route('result-chain.program-output-indicators.create')" class="btn btn-success w-100">
                                <i class="bi bi-plus-circle me-1"></i> Add PPOI
                            </Link>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <tr v-if="!indicators?.data?.length"><td colspan="5" class="text-center text-muted py-4">No program output indicators found</td></tr>
                            <tr v-for="ind in indicators?.data" :key="ind.id">
                                <td><span class="badge badge-ppoi">{{ ind.code }}</span></td>
                                <td>{{ ind.title }}</td>
                                <td><small>{{ ind.program ? `${ind.program.code} — ${ind.program.title}` : '—' }}</small></td>
                                <td><span class="badge bg-secondary">{{ ind.outcome_indicators_count ?? 0 }}</span></td>
                                <td>
                                    <div class="btn-group">
                                        <Link :href="route('result-chain.program-output-indicators.edit', ind.id)" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></Link>
...
.badge-ppoi { background-color: #e65100; color: #fff; }
