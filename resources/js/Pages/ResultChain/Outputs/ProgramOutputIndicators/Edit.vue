<script setup>
import { ref, computed } from 'vue'
import { useForm, Head, Link } from '@inertiajs/vue3'
import BeLayout from '../../../../Layouts/BeLayout.vue'

const props = defineProps({ indicator: Object, selectedIds: Array, programs: Array, outcomeIndicators: Array })

const form = useForm({
    title: props.indicator.title,
    description: props.indicator.description ?? '',
    program_id: props.indicator.program_id ?? '',
    outcome_indicator_ids: [...props.selectedIds],
})

const search = ref('')
const filtered = computed(() => {
    const q = search.value.toLowerCase()
    return q ? props.outcomeIndicators.filter(i => i.code.toLowerCase().includes(q) || i.title.toLowerCase().includes(q)) : props.outcomeIndicators
})

const submit = () => form.put(route('result-chain.program-output-indicators.update', props.indicator.id))
</script>

<template>
    <BeLayout>
        <Head title="Edit Program Output Indicator" />
        <h5 class="mt-4">Edit Program Output Indicator</h5>
        <hr />

        <div class="row">
            <div class="col-lg-9">
                <div class="card shadow-sm">
                    <div class="card-header card-header-green text-white">
                        <h6 class="mb-0"><i class="bi bi-folder-check me-2"></i>Edit PPOI — <span class="opacity-75">{{ indicator.code }}</span></h6>
                    </div>
                    <div class="card-body">
                        <form @submit.prevent="submit">
                            <div class="row mb-3">
                                <div class="col-md-2">
                                    <label class="form-label">Code</label>
                                    <div class="pt-1"><span class="badge bg-secondary fs-6">{{ indicator.code }}</span></div>
                                </div>
                                <div class="col-md-5">
                                    <label class="form-label">Program</label>
                                    <select v-model="form.program_id" class="form-select">
                                        <option value="">— Select program —</option>
                                        <option v-for="p in programs" :key="p.id" :value="p.id">{{ p.code }} — {{ p.title }}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Title <span class="text-danger">*</span></label>
                                <input v-model="form.title" type="text" class="form-control"
                                    :class="{ 'is-invalid': form.errors.title }" required />
                                <div class="invalid-feedback">{{ form.errors.title }}</div>
                            </div>
                            <div class="mb-4">
                                <label class="form-label">Description</label>
                                <textarea v-model="form.description" class="form-control" rows="3" />
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-semibold">Linked Outcome Indicators</label>
                                <div class="input-group input-group-sm mb-2">
                                    <span class="input-group-text"><i class="bi bi-search"></i></span>
                                    <input v-model="search" type="text" class="form-control" placeholder="Filter..." />
                                </div>
                                <div class="indicator-list border rounded p-2">
                                    <div v-if="!filtered.length" class="text-muted small p-2">No outcomes match.</div>
                                    <div v-for="ind in filtered" :key="ind.id"
                                        class="form-check py-1 px-3 indicator-item"
                                        :class="{ selected: form.outcome_indicator_ids.includes(ind.id) }">
                                        <input class="form-check-input" type="checkbox"
                                            :id="`out-${ind.id}`" :value="ind.id" v-model="form.outcome_indicator_ids" />
                                        <label class="form-check-label w-100" :for="`out-${ind.id}`">
                                            <span class="badge badge-out me-2">{{ ind.code }}</span>
                                            <span class="small">{{ ind.title }}</span>
                                        </label>
                                    </div>
                                </div>
                                <div class="text-muted small mt-1">{{ form.outcome_indicator_ids.length }} selected</div>
                            </div>
                            <div class="d-flex justify-content-between">
                                <Link :href="route('result-chain.program-output-indicators.index')" class="btn btn-secondary">
                                    <i class="bi bi-arrow-left me-2"></i>Cancel
                                </Link>
                                <button type="submit" class="btn btn-success" :disabled="form.processing">
                                    <i class="bi bi-save me-2"></i>Update PPOI
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </BeLayout>
</template>

<style scoped>
.card-header-green { background-color: rgb(11,109,23); }
.btn-success { background-color: rgb(11,109,23); border-color: rgb(11,109,23); }
.btn-success:hover { background-color: rgb(9,87,18); border-color: rgb(9,87,18); }
.badge-out { background-color: #1565c0; color: #fff; font-size: .75rem; padding: .25em .5em; border-radius: .25rem; }
.indicator-list { max-height: 300px; overflow-y: auto; }
.indicator-item { border-radius: 4px; cursor: pointer; }
.indicator-item:hover { background-color: rgba(11,109,23,.06); }
.indicator-item.selected { background-color: rgba(11,109,23,.12); }
</style>
