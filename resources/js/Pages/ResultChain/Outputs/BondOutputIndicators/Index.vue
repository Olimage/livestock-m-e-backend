...
watch([search, perPage, sortBy, sortOrder], () => {
    ...
    t.value = setTimeout(() => {
        router.get(route('result-chain.bond-output-indicators.index'), {
...
const del = (id) => {
    if (confirm('Delete this Bond Output Indicator?'))
        router.delete(route('result-chain.bond-output-indicators.destroy', id), { preserveScroll: true })
}
...
<template>
    <BeLayout>
        <Head title="Bond Output Indicators" />
        <h5 class="mt-4 fw-400">Bond Output Indicators</h5>
        <hr />
        <p class="text-muted">Prefix: <span class="badge badge-boi">BOI</span> &nbsp; Total: <strong>{{ totalCount }}</strong></p>

        <div class="row card mb-3">
            <div class="col-sm-12">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4"><div class="card stat-card mb-3"><div class="card-header">Total BOI</div><div class="card-body"><h3 class="text-white">{{ totalCount }}</h3></div></div></div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-3 ms-auto mb-2">
                            <Link :href="route('result-chain.bond-output-indicators.create')" class="btn btn-success w-100">
                                <i class="bi bi-plus-circle me-1"></i> Add BOI
                            </Link>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            <tr v-if="!indicators?.data?.length"><td colspan="4" class="text-center text-muted py-4">No bond output indicators found</td></tr>
                            <tr v-for="ind in indicators?.data" :key="ind.id">
                                <td><span class="badge badge-boi">{{ ind.code }}</span></td>
                                <td>{{ ind.title }}</td>
                                <td><span class="badge bg-secondary">{{ ind.outcome_indicators_count ?? 0 }}</span></td>
                                <td>
                                    <div class="btn-group">
                                        <Link :href="route('result-chain.bond-output-indicators.edit', ind.id)" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></Link>
...
.badge-boi { background-color: #6a1b9a; color: #fff; }
