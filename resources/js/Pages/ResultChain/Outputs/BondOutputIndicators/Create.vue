...
const props = defineProps({ outcomeIndicators: Array })

const form = useForm({
    title: '', description: '', outcome_indicator_ids: [],
})
...
const submit = () => form.post(route('result-chain.bond-output-indicators.store'))
</script>

<template>
    <BeLayout>
        <Head title="Create Bond Output Indicator" />
        <h5 class="mt-4">Create Bond Output Indicator</h5>
        <hr />
            ...
            <div class="col-lg-9">
                <div class="card shadow-sm">
                    <div class="card-header card-header-green text-white">
                        <h6 class="mb-0"><i class="bi bi-bookmark-check me-2"></i>New Bond Output Indicator <span class="badge bg-light" style="color:#6a1b9a">BOI</span></h6>
                    </div>
                    <div class="card-body">
                        <!-- Code is auto-generated (BOI-{id}) — not shown on create -->
                        <form @submit.prevent="submit">
                            ...
                            <div class="d-flex justify-content-between">
                                <Link :href="route('result-chain.bond-output-indicators.index')" class="btn btn-secondary">
                                    <i class="bi bi-arrow-left me-2"></i>Cancel
                                </Link>
                                <button type="submit" class="btn btn-success" :disabled="form.processing">
                                    <i class="bi bi-save me-2"></i>Create Bond Output Indicator
                                </button>
                            </div>
                        </form>
                    </div>
