...
class BondDeliverableSeeder extends Seeder
{
    ...
    {
        $indicatorMap = BondOutputIndicator::whereIn('code', collect($this->deliverables)->pluck('indicators')->flatten()->unique()->values()->toArray())
            ->pluck('id', 'code');
        ...
        foreach ($this->deliverables as $row) {
            ...
            if (! empty($ids)) {
                $bd->bondOutputIndicators()->syncWithoutDetaching($ids);
            }
        }
